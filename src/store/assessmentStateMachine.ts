...
export const useAssessmentStateMachine = create<AssessmentStateMachineStore>((set, get) => ({
  ...
  startAnswering: () => set({ state: 'answering' }),
  ...
  saveAnswer: async (questionId: string, optionId: string, value: number) => {
    ...
    try {
      ...
      const storageKey = 'mm_ans_' + session.session_id
  ...
  setError: (error: string | null) => set({ error, state: error ? 'error' : get().state }),
  ...
  setAnswering: () => set({ state: 'answering' }),
}))
