...
| 8 套题库 | ✅ 已完成 | 内容就绪，等待接入 |
| 本地数据闭环 | ✅ 已完成 | 结果持久化、草稿箱、历史记录、设置中心 |
...
- **本地优先数据** - 所有数据存储在浏览器 IndexedDB/localStorage 中（无需后端）
- **响应式设计** - 移动端友好，支持深色模式
- **3D 视觉体验** - Three.js 沉浸式背景效果
- **流畅动画** - Framer Motion 驱动的可配置动画级别
- **多维度分析图表** - 雷达图、柱状图、分布图可视化
- **分类组织** - 5 大测评分类体系
- **草稿自动保存** - 答题进度自动保存并可恢复
- **个人数据中心** - 历史记录、草稿箱、设置中心
- **主题与偏好** - 浅色/深色/跟随系统主题、字号、动画级别控制
...
| 数据类型 | 存储方式 | 说明 |
|----------|----------|------|
| 答题结果 | Dexie (IndexedDB) | 通过 `db.results` 表持久化存储 |
| 答题草稿 | Dexie (IndexedDB) | 通过 `db.drafts` 表自动保存进度 |
| 用户设置 | Zustand + Dexie | 通过 `db.settings` 表双重持久化 |
| 用户画像 | Dexie (IndexedDB) | 通过 `db.profile` 表存储 |

### 数据流程

```
应用初始化
  → initializeDatabase() [打开 Dexie 数据库]
  → initializeSettings() [空则写入默认值]
  → settingsStore.syncFromDB() [加载设置到 Store]

答题流程
  → 用户开始答题 → quizStore 初始化
  → 答案保存到 quizStore（内存）
  → handleAutoSave() 每 2 秒保存草稿到 Dexie（如果已启用）
  → beforeunload 保存草稿备份到 localStorage
  → 答题完成 → 结果保存到 Dexie
  → 提交成功后自动删除草稿

个人中心流程
  → syncProfileFromResults() 聚合数据
  → 历史记录、草稿箱、设置都从 Dexie 读取
```

### 无后端

这是一个纯静态应用。所有数据都保存在用户的浏览器中（通过 IndexedDB/Dexie）。目前没有：
- 用户认证系统
- 云端同步功能
- 服务端数据处理
- 数据与特定浏览器/设备绑定
...
2. **仅本地存储**：历史记录、草稿箱、设置都与浏览器/设备绑定，无云端同步
3. **无用户系统**：纯静态应用，无登录注册功能
4. **无 AI 分析**：报告为预定义（基于评分规则），非 AI 生成
5. **草稿自动保存需要开启 `autoSaveDraft`**：草稿保存由用户设置控制
...
- 无需后端服务器
- 无需数据库服务器
- 所有数据存储在浏览器 IndexedDB/localStorage 中（通过 Dexie）
